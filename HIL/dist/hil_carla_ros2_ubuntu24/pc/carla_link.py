...
class CarlaLink:
    def __init__(self, carla, host, port, scenario, town=None,
        ...
        self._lead_actor_type = 'vehicle'
        self._lead_initial_lateral_offset = 0.0
        ...
        if lead_cfg:
            ...
            self._lead_actor_type = str(lead_cfg.get('actor', 'vehicle'))
            lateral = float(lead_cfg.get('lateral_offset', 0.0))
            self._lead_initial_lateral_offset = lateral
            transform = self._offset_transform(lead_wps[0].transform, lateral)
            if self._lead_actor_type == 'pedestrian':
                bp_name = str(lead_cfg.get('blueprint', 'walker.pedestrian.*'))
            else:
                bp_name = str(lead_cfg.get('blueprint', LEAD_BLUEPRINT))
            self.lead = self._spawn_at(bp_name, 'lead', transform,
                                       actor_type=self._lead_actor_type)
    ...
    def _offset_transform(self, transform, lateral_offset):
        if not lateral_offset:
            return transform
        yaw = math.radians(float(transform.rotation.yaw))
        loc = self.carla.Location(
            x=float(transform.location.x - math.sin(yaw) * lateral_offset),
            y=float(transform.location.y + math.cos(yaw) * lateral_offset),
            z=float(transform.location.z))
        rot = self.carla.Rotation(
            pitch=float(transform.rotation.pitch),
        ...
        if self.lead is not None:
            ...
            if 0.5 < forward < 120.0:
                frame.update({
                    ...
                    'lead_cls': int(self._lead_cfg.get('cls', 1)),
                })
    ...
    def drive_lead(self, sim_t):
        ...
        if str(lead_cfg.get('actor', 'vehicle')) == 'pedestrian':
            self._drive_pedestrian(lead_cfg)
            return
        ...
        if lead_cfg.get('cut_in') and sim_t >= float(lead_cfg.get('cut_in_at', 3.0)):
            duration = max(0.2, float(lead_cfg.get('cut_in_duration', 2.5)))
            if sim_t <= float(lead_cfg.get('cut_in_at', 3.0)) + duration:
                direction = -1.0 if self._lead_initial_lateral_offset > 0.0 else 1.0
                steer = _clamp(steer + direction * float(lead_cfg.get('cut_in_steer', 0.35)),
                               -0.65, 0.65)
        self.lead.apply_control(self.carla.VehicleControl(
            throttle=float(throttle), brake=float(brake),
            steer=steer))

    def _drive_pedestrian(self, lead_cfg):
        yaw = math.radians(float(self._ref_wp.transform.rotation.yaw))
        direction = 1.0 if self._lead_initial_lateral_offset < 0.0 else -1.0
        vx = -math.sin(yaw) * direction
        vy = math.cos(yaw) * direction
        speed = float(lead_cfg.get('cross_speed', 1.6))
        self.lead.apply_control(self.carla.WalkerControl(
