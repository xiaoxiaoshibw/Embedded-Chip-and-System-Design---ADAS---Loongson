...
# ── ML / 锁步守护进程连接配置 ──
# ml_inferd 和 lockstepd 由 ADAS.py main() 自动启动为独立进程，
# 分别绑到 ML_INFERD_CORE（默认 3）和 LOCKSTEPD_CORE（默认 2）。
# 端口可通过环境变量覆盖，避免与 PC 侧端口冲突。
ML_INFERD_HOST = os.environ.get('ML_INFERD_HOST', '127.0.0.1')
ML_INFERD_PORT = int(os.environ.get('ML_INFERD_PORT', '19999'))
ML_INFERD_CORE = int(os.environ.get('ML_INFERD_CORE', '3'))

LOCKSTEPD_HOST = os.environ.get('LOCKSTEPD_HOST', '127.0.0.1')
LOCKSTEPD_PORT = int(os.environ.get('LOCKSTEPD_PORT', '19998'))
LOCKSTEPD_CORE = int(os.environ.get('LOCKSTEPD_CORE', '2'))
